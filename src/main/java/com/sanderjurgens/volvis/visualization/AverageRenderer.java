...
/**
 * A renderer that uses ray casting to compute the average intensity projection
 * of the volume.
 *
 * @author sanderjurgens
 */
public class AverageRenderer extends RaycastingRenderer {

    /**
     * Constructs a renderer to visualize the volume based on ray casting to
     * compute the average intensity projection.
     *
     * @param name the name of the renderer
     * @param renderingTab the tab containing the rendering settings
     */
    public AverageRenderer(String name, RenderingTab renderingTab) {
        ...
        for (int j = startHeight; j <= endHeight; j = j + res) {
            for (int i = 0; i < getImage().getWidth(); i = i + res) {
                ...
                for (int k = 0; k < diagonal; k = k + ss) {
                    ...
                    // Only add value to total if coordinate lies within volume
                    if (0 <= Math.floor(volumeCoord[0]) && Math.ceil(volumeCoord[0]) < getVolume().getDimX()
                            && 0 <= Math.floor(volumeCoord[1]) && Math.ceil(volumeCoord[1]) < getVolume().getDimY()
                            && 0 <= Math.floor(volumeCoord[2]) && Math.ceil(volumeCoord[2]) < getVolume().getDimZ()) {
                        val = val + getTriVoxel(volumeCoord);
                        samples++;
                    }
                }
                
                // Compute average value
                val = (int) ((double) val / (double) samples);
